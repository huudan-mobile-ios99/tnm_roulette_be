...
router.route('/home').get((req, res) => {
    return res.status(200).json('TNM v3');
})
...
router.route('/save_list_station').get(async (req, res) => {
    try {
        dboperation.allStationData(async (err, result) => {
            if (err) {
                return res.status(500).json({ error: 'Error fetching all station data.' });
            }
            const today = new Date();
            const formattedDate = `${today.getDate().toString().padStart(2, '0')}${today.toLocaleString('en-US', { month: 'short' }).toUpperCase()}`;

            const existingRounds = await rankingRealtimeModel.find({ roundName: { $regex: `^RL_${formattedDate}_` } });
            const roundIndex = existingRounds.length + 1;
            const roundNameUnique = `RL_${formattedDate}_${roundIndex}`;
            ...
            await mongodbFunction.saveListStationToMongo(extractedData, roundNameUnique);
