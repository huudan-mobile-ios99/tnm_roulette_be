...
router.route('/create_round').post(async (req, res) => {
  try {
    ...
    const lastRound = await roundModel.findOne({ name: new RegExp(`^RL_${formattedDate}_\\d+$`) }).sort({ createdAt: -1 });
    let roundNumber = 1;

    if (lastRound) {
      const match = lastRound.name.match(/(\d+)$/);
      if (match) {
        roundNumber = parseInt(match[1]) + 1;
      }
    }

    const newRound = new roundModel({
      id: randongString.generate(),
      name: `RL_${formattedDate}_${roundNumber}`,
      rankings: savedRankingIds,
    });
    ...
    res.status(500).json({
      status: false,
      message: 'An error occurred while creating rounds',
      error: err.message,
    });
  }
});
